...
namespace game
{
	struct MainLevel final : Level
	{
		...
		struct CombatState final : LevelState<State>
		{
			...
			float previousDialogueTime;
			float previousDialogueDuration;
			...
			void PickRandomMonsterDialogue(State& state, const LevelUpdateInfo& info, const Level* level, const ActionState& actionState);
			...
			void DrawParallaxBackground(const Level& level, State& state, const LevelUpdateInfo& info, bool mirror) const;
		};
